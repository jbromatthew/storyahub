          ...
          {(() => {
            ...
            return (
              <div className={"inc-banner " + (ok ? "ok" : "no")}>
                <div className="lead">
                  <span className={"inc-state " + (ok ? "ok" : "no")}>
            ...
            {live.distribution.map((row) => {
              ...
              return (
                <div key={row.name} className={"inc-card" + (row.top ? " top" : "")}>
                  <div className="who">
                    <AssigneeBadge name={row.name} />
                    <span className={"inc-rank" + (row.top ? " top" : "")}>
                      {sc ? `${sc.rank}위` : "영업지원"}
                    </span>
                  </div>
          ...
          <div className="card" style={{ marginTop: 20, padding: "16px 18px" }}>
            ...
            <div className="dash-table-wrap">
              <table className="inc-nbm">
                ...
                <tbody>
                  {[
                    { label: "이카운트 ERP (HW매출)", draft: hwDraft, set: setHwDraft, total: hwTotal },
                    { label: "렌탈 매출", draft: rentalDraft, set: setRentalDraft, total: rentalTotal },
                  ].map((row) => (
                    <tr key={row.label}>
                      <td>{row.label}</td>
                      {data.months.map((m, i) => (
                ...
                <tfoot>
                  <tr>
                    <td>NBM 합계</td>
                    {[0, 1, 2].map((i) => {
                      const v = (Number(String(hwDraft[i]).replace(/[^0-9]/g, "")) || 0)
                        + (Number(String(rentalDraft[i]).replace(/[^0-9]/g, "")) || 0);
                      return <td key={i}>{v ? formatWon(v) : "-"}</td>;
                    })}
                    <td>{nbmTotal ? formatWon(nbmTotal) : "-"}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
    ...
    for (const [segKey, segLabel] of DROP_SEGS) {
      ...
      for (const m of DROP_METRICS) {
        ...
        if (m.type === "rate") {
          ...
          // 비교 기준 쪽 표본이 너무 작으면 % 자체가 불안정 — 문의 합 5건 미만 제외
      ...
      {tab === "applies" && (
        <>
          <div className="trend-selection-bar" style={{ marginTop: 14 }}>
            ...
            <span>세일즈 <strong>{applies.filter((a) => a.source === "sales").length}</strong></span>
...
// 골프(별도 계산식): 기본설치비 500,000 + 타석당 15,000 · 제주 출장비 성수기(4~10월) 500,000 / 비수기 350,000
...
export function installSettleCalc(row) {
  ...
  const mainUnits = []; // 70% 규칙 대상 (키오스크 본체만 — 골프·IoT는 각자 계산식)
  ...
  for (const it of items) {
    ...
    // 골프: 기본설치비 500,000 + 타석당 15,000 — 이게 전부 (키오스크 70% 규칙과 무관, 프로그램 지급 없음)
    if (/스크린/.test(n)) { golfScreen = true; parts.push({ label: `${n} ×${it.qty} (골프 기본설치비)`, amount: 500000 * it.qty }); continue; }
    if (/프로그/.test(n)) { parts.push({ label: `${n} (지급 없음)`, amount: 0 }); continue; }
    if (/타석/.test(n)) { needGolfScreen = true; parts.push({ label: `${n} ×${it.qty} (타석당 1.5만)`, amount: 15000 * it.qty }); continue; }
    if (/골프/.test(n)) { golfScreen = true; parts.push({ label: `${n} ×${it.qty} (골프 기본설치비)`, amount: 500000 * it.qty }); continue; }
    ...
    if (/인치|QR|Q-?PASS|키오스크|DID/i.test(n)) {
  ...
  // 골프 타석만 있고 기본설치비 항목이 없으면 1식(500,000) 자동 추가
  if (needGolfScreen && !golfScreen) parts.push({ label: "골프 기본설치비 (1식)", amount: 500000 });
  ...
  // 키오스크 2대 이상: 단가 높은 것 100%, 나머지 70% (키오스크끼리만 적용)
  mainUnits.sort((a, b) => b.unit - a.unit);
  mainUnits.forEach((u, idx) => {
    parts.push({
      label: `${u.name} (${region}${idx > 0 ? " · 추가 70%" : ""})`,
      amount: idx === 0 ? u.unit : Math.round(u.unit * 0.7),
    });
  });
...
function SalesCaseFeed({ articles, openWrite }) {
  ...
  const stats = (() => {
    ...
    for (const c of meta.cases) {
      const s = m.get(c.authorName) || { name: c.authorName, total: 0, ok: 0 };
      s.total += 1; if (c.cooApproved) s.ok += 1;
      m.set(c.authorName, s);
    }
    ...
    <>
      ...
      <div className="card" style={{ marginTop: 14, padding: "16px 18px", maxWidth: 720 }}>
        ...
        <ul className="oa-notes">
          ...
          <li><b>발급 요청 조회 API는 아직 서버에 없습니다</b> (404). 문서에는 있지만 배포 전이라, 「발급 요청」 탭과 「발급 요청에서 센터 가져오기」는 배포된 뒤에 됩니다. 발급·재발급·정지·폐기·연결 센터는 지금 다 됩니다.</li>
