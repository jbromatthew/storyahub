...
/* 세일즈 통계 — 직전서비스 비교 */
.rate-groups{display:grid;grid-template-columns:repeat(auto-fit,minmax(236px,1fr));gap:10px}
.ps-list{display:flex;flex-direction:column;gap:2px}
.ps-row2{display:grid;grid-template-columns:26px minmax(100px,158px) 1fr;gap:10px;
  align-items:center;padding:9px 0;border-bottom:1px solid var(--line-2,#F0EBE1)}
.ps-row2:last-child{border-bottom:0}
.ps-rank{font-size:11.5px;color:var(--muted);text-align:right;font-variant-numeric:tabular-nums}
...
.ps-barline b{text-align:right;font-weight:800;font-size:12.5px;font-variant-numeric:tabular-nums}
.ps-barline em{text-align:right;font-style:normal;font-size:11.5px;color:var(--muted);
  font-variant-numeric:tabular-nums}
@media(max-width:720px){
  .ps-row2{grid-template-columns:22px minmax(0,1fr);row-gap:6px}
  .ps-bars{grid-column:1 / -1}
}
...
/* ── 인센티브 ── */
.inc-banner{display:flex;gap:18px;align-items:center;flex-wrap:wrap;border-radius:16px;padding:18px 20px;margin-top:14px;border:1px solid var(--line);background:var(--card);}
.inc-banner.ok{border-color:#A7E3C0;}
.inc-banner.no{border-color:#F3C9C5;}
.inc-banner .lead{flex:1 1 260px;min-width:0;}
.inc-state{display:inline-flex;align-items:center;gap:6px;font-size:12.5px;font-weight:800;border-radius:999px;padding:5px 12px;}
.inc-state.ok{background:#D3F8DF;color:#1F6B3A;}
.inc-state.no{background:#FBE2DF;color:#B3261E;}
.inc-bar{height:9px;border-radius:99px;background:#EDEFF2;overflow:hidden;margin-top:10px;}
.inc-bar>i{display:block;height:100%;border-radius:99px;background:#1F6B3A;}
.inc-bar.no>i{background:#C5221F;}
.inc-pool{text-align:right;flex:0 0 auto;}
.inc-pool .n{font-size:26px;font-weight:800;letter-spacing:-.02em;}

.inc-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:12px;margin-top:14px;}
.inc-card{position:relative;border:1px solid var(--line);border-radius:16px;padding:16px;background:var(--card);}
.inc-card.top{border-color:#F5A15E;box-shadow:0 6px 20px rgba(250,100,0,.12);}
.inc-card .who{display:flex;align-items:center;gap:6px;flex-wrap:wrap;}
.inc-rank{font-size:11px;font-weight:800;border-radius:999px;padding:3px 9px;background:#EEF0F3;color:var(--muted);}
.inc-rank.top{background:#FA6400;color:#fff;}
.inc-amt{font-size:23px;font-weight:800;letter-spacing:-.02em;margin-top:10px;}
.inc-amt.off{color:var(--muted);font-size:18px;}
.inc-share{font-size:12px;color:var(--muted);margin-top:2px;}
.inc-sep{border-top:1px solid var(--line);margin:12px 0 10px;}
.inc-metric{display:flex;justify-content:space-between;gap:8px;font-size:12.5px;margin-bottom:5px;align-items:center;}
.inc-metric .k{color:var(--muted);}
.inc-metric .v{font-weight:700;font-variant-numeric:tabular-nums;}
.inc-score{height:6px;border-radius:99px;background:#EDEFF2;overflow:hidden;margin:8px 0 4px;}
.inc-score>i{display:block;height:100%;border-radius:99px;background:#8A93A1;}
.inc-card.top .inc-score>i{background:#FA6400;}
.inc-lead-in{width:52px;border:1px solid var(--line);border-radius:8px;padding:4px 7px;font-family:inherit;font-size:13px;text-align:right;}

.inc-nbm{width:100%;border-collapse:collapse;margin-top:4px;}
.inc-nbm th,.inc-nbm td{padding:8px 10px;border-bottom:1px solid var(--line);font-size:13px;text-align:right;}
.inc-nbm th:first-child,.inc-nbm td:first-child{text-align:left;white-space:nowrap;}
.inc-nbm thead th{font-size:11.5px;color:var(--muted);font-weight:700;}
.inc-nbm tfoot td{font-weight:800;border-bottom:none;}
.inc-nbm input{width:100%;min-width:96px;border:1px solid var(--line);border-radius:9px;padding:7px 9px;font-family:inherit;font-size:13.5px;text-align:right;}
/* 인센티브 — 숫자를 누르면 그 뒤의 결제 건을 펼친다 */
.inc-drill-btn{border:none;background:none;padding:0;font:inherit;font-weight:700;font-variant-numeric:tabular-nums;color:var(--ink);cursor:pointer;border-bottom:1px dashed var(--line);}
.inc-drill-btn:hover{color:#FA6400;border-bottom-color:#FA6400;}
.inc-drill-name{border:none;background:none;padding:0;font:inherit;cursor:pointer;}
.inc-drill-name:hover{opacity:.7;}
.inc-drill-bg{position:fixed;inset:0;z-index:380;background:rgba(20,16,12,.42);display:flex;align-items:flex-start;justify-content:center;padding:5vh 14px;overflow:auto;}
.inc-drill{width:100%;max-width:760px;background:var(--card,#fff);border:1px solid var(--line);border-radius:18px;padding:16px 18px 20px;box-shadow:0 18px 48px rgba(0,0,0,.2);animation:fadeUp .2s ease both;}
.inc-drill-hd{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:12px;}
.inc-drill-x{border:none;background:var(--paper,#F5F3EF);width:32px;height:32px;border-radius:10px;cursor:pointer;color:var(--muted);font-size:14px;}
.inc-drill-x:hover{background:var(--line);color:var(--ink);}
.inc-drill-sum{display:flex;gap:10px;flex-wrap:wrap;}
.inc-drill-sum>div{flex:1 1 150px;border:1px solid var(--line);border-radius:12px;padding:10px 12px;}
.inc-drill-sum .k{display:block;font-size:11.5px;color:var(--muted);font-weight:700;}
.inc-drill-sum .v{display:block;font-size:17px;font-weight:800;letter-spacing:-.02em;margin-top:3px;font-variant-numeric:tabular-nums;}
.inc-drill-sum .v.off{color:var(--muted);text-decoration:line-through;}
.inc-drill-tbl{width:100%;border-collapse:collapse;font-size:13px;min-width:560px;}
.inc-drill-tbl th,.inc-drill-tbl td{padding:8px 10px;border-bottom:1px solid var(--line);text-align:center;white-space:nowrap;}
.inc-drill-tbl thead th{font-size:11.5px;color:var(--muted);font-weight:700;background:var(--paper,#F5F3EF);position:sticky;top:0;}
.inc-drill-tbl th.l,.inc-drill-tbl td.l{text-align:left;white-space:normal;}
.inc-drill-tbl th.r,.inc-drill-tbl td.r{text-align:right;font-variant-numeric:tabular-nums;font-weight:700;}
.inc-drill-tbl td.dt{color:var(--muted);font-variant-numeric:tabular-nums;}
.inc-drill-tbl tr.pub td{background:#FFF7ED;color:var(--muted);}
.inc-drill-tbl tr.pub td.r{text-decoration:line-through;}
.inc-drill-tbl .sub{color:var(--muted);font-weight:500;font-size:12px;}
.inc-drill-kind{display:inline-block;font-size:11px;font-weight:700;border-radius:999px;padding:2px 8px;background:#EEF0F3;color:#4A5160;}
.inc-drill-ex{display:inline-block;margin-left:6px;font-size:10.5px;font-weight:800;border-radius:999px;padding:2px 7px;background:#FBE2DF;color:#B3261E;}

/* ===== 파트너스 ===== */
.pt-tiers{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px;margin-top:16px;}
.pt-tier{text-align:left;border:1px solid var(--line);border-radius:14px;padding:13px 14px;background:var(--card,#fff);cursor:pointer;font:inherit;transition:border-color .15s,box-shadow .15s;}
.pt-tier:hover{border-color:#C9CDD4;}
.pt-tier.on{box-shadow:0 0 0 2px rgba(250,100,0,.18);}
.pt-tier-hd{display:flex;align-items:center;justify-content:space-between;gap:8px;}
.pt-tier-name{font-weight:800;font-size:14.5px;letter-spacing:-.01em;}
.pt-tier-n{font-size:16px;font-weight:800;font-variant-numeric:tabular-nums;}
.pt-tier-desc{font-size:12px;color:var(--muted);margin-top:6px;line-height:1.45;}
.pt-tier-term{font-size:11.5px;color:var(--muted);margin-top:6px;padding-top:6px;border-top:1px solid var(--line);}
.pt-tier.pt-core{border-left:3px solid #FA6400;}
.pt-tier.pt-growth{border-left:3px solid #2E7D6B;}
.pt-tier.pt-spec{border-left:3px solid #5B6ABF;}
.pt-chips{display:flex;gap:6px;flex-wrap:wrap;margin-top:12px;}
.chip.sm{font-size:11.5px;padding:4px 9px;}
.pt-search{flex:1 1 260px;min-width:0;border:1px solid var(--line);border-radius:10px;padding:8px 12px;font-family:inherit;font-size:13.5px;background:#fff;}
.pt-wrap{overflow-x:auto;border:1px solid var(--line);border-radius:12px;background:#fff;-webkit-overflow-scrolling:touch;}
.pt-tbl{width:100%;border-collapse:separate;border-spacing:0;font-size:13px;min-width:1000px;}
.pt-tbl th,.pt-tbl td{padding:9px 11px;border-bottom:1px solid var(--line);text-align:center;white-space:nowrap;vertical-align:middle;}
.pt-tbl thead th{position:sticky;top:0;z-index:1;background:var(--paper,#F5F3EF);font-size:11.5px;font-weight:700;color:var(--muted);cursor:pointer;user-select:none;}
.pt-tbl thead th:hover{color:var(--ink);}
.pt-tbl thead th.on{color:var(--ink);}
.pt-tbl th.l,.pt-tbl td.l{text-align:left;}
.pt-tbl tbody tr{cursor:pointer;}
.pt-tbl tbody tr:hover td{background:var(--accent-soft,#FFF4EC);}
.pt-tbl tbody tr:last-child td{border-bottom:none;}
.pt-sort{display:inline-block;width:12px;font-size:9px;color:var(--muted);}
.pt-name{font-weight:700;}
.pt-sub{color:var(--muted);font-weight:500;font-size:12px;}
.pt-cat{font-size:12px;color:var(--muted);}
.pt-feature{max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--muted);}
.pt-badge{display:inline-block;font-size:11px;font-weight:800;border-radius:999px;padding:3px 9px;}
.pt-badge.pt-core{background:#FFE8D6;color:#B24B00;}
.pt-badge.pt-growth{background:#D9F2EA;color:#1F6B57;}
.pt-badge.pt-spec{background:#E3E7FA;color:#3D4A9E;}
.pt-badge.pt-none{background:#EEF0F3;color:#8A93A1;}
.pt-st{display:inline-block;font-size:11px;font-weight:800;border-radius:999px;padding:3px 9px;}
.pt-st-gray{background:#EEF0F3;color:#6B7280;}
.pt-st-blue{background:#E0EDFB;color:#1F5FA8;}
.pt-st-amber{background:#FDF0D5;color:#8A5512;}
.pt-st-violet{background:#EDE6FB;color:#5B37B0;}
.pt-st-green{background:#D3F8DF;color:#1F6B3A;}
.pt-st-slate{background:#E6E9EE;color:#4A5160;}
...
/* 고객관리 · 센터조회 — CRM 목록 화면 결에 맞춘다 */
.cc-segbar{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:14px;padding:10px 12px;
  border:1px solid var(--line,#E5E7EB);border-radius:12px;background:var(--surface-2,#FAFAFA)}
.cc-seglabel{font-size:11.5px;font-weight:700;letter-spacing:.12em;color:var(--muted)}
.cc-seg{display:inline-flex;align-items:stretch;border:1px solid var(--line,#E5E7EB);border-radius:999px;
  overflow:hidden;background:#fff}
.cc-seg.on{border-color:#FA6400;box-shadow:0 0 0 1px #FA6400 inset}
.cc-seg-main{border:0;background:transparent;padding:5px 12px;font-size:12.5px;cursor:pointer;
  display:inline-flex;align-items:center;gap:6px;color:inherit}
.cc-seg.on .cc-seg-main{color:#C24C00;font-weight:600}
.cc-seg-main:hover{background:var(--surface-2,#F5F6F8)}
.cc-seg-share{font-size:10px;padding:1px 5px;border-radius:6px;background:#EEF3FF;color:#33529E}
.cc-seg-x{border:0;border-left:1px solid var(--line,#E5E7EB);background:transparent;
  padding:0 7px;font-size:11px;color:var(--muted);cursor:pointer}
.cc-seg-x:hover{background:#FBEAEA;color:#A33}

.cc-chips{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:12px}
.cc-chip{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--line,#E5E7EB);
  background:#fff;border-radius:999px;padding:6px 13px;font-size:12.5px;cursor:pointer;color:inherit;
  white-space:nowrap}
.cc-chip.sm{padding:4px 10px;font-size:12px}
.cc-chip:hover{background:var(--surface-2,#F5F6F8)}
.cc-chip.on{border-color:#FA6400;color:#C24C00;font-weight:600;background:#FFF6EF}
.cc-cnt{font-size:11.5px;color:var(--muted);font-variant-numeric:tabular-nums}
.cc-chip.on .cc-cnt{color:#C24C00}
.cc-div{width:1px;height:18px;background:var(--line,#E5E7EB);margin:0 4px}

.cc-toolbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:12px}
.cc-search{position:relative;flex:0 1 320px;min-width:220px}
.cc-search input{width:100%;border:1px solid var(--line,#E5E7EB);border-radius:10px;
  padding:8px 30px 8px 32px;font-size:13px;background:#fff;color:inherit}
.cc-search input:focus{outline:none;border-color:#FA6400;box-shadow:0 0 0 3px rgba(250,100,0,.12)}
.cc-search-i{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:15px}
.cc-search-x{position:absolute;right:8px;top:50%;transform:translateY(-50%);border:0;background:transparent;
  color:var(--muted);cursor:pointer;font-size:11px}
.cc-btn{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--line,#E5E7EB);background:#fff;
  border-radius:10px;padding:8px 13px;font-size:12.5px;cursor:pointer;color:inherit;white-space:nowrap}
.cc-btn:hover{background:var(--surface-2,#F5F6F8)}
.cc-btn.on{border-color:#FA6400;color:#C24C00;font-weight:600;background:#FFF6EF}
.cc-total{font-size:12.5px;color:var(--muted)}
.cc-total b{color:var(--text,#1A1A1A);font-variant-numeric:tabular-nums}
.cc-tools{margin-left:auto;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.cc-icon{width:32px;height:32px;border:1px solid var(--line,#E5E7EB);background:#fff;border-radius:9px;
  cursor:pointer;color:var(--muted);font-size:14px;line-height:1;display:inline-flex;
  align-items:center;justify-content:center}
.cc-icon:hover:not(:disabled){background:var(--surface-2,#F5F6F8);color:inherit}
.cc-icon:disabled{opacity:.4;cursor:default}

.cc-pill{display:inline-block;padding:2px 9px;border-radius:999px;font-size:11.5px;font-weight:600}
.cc-pill.ok{background:#D3F8DF;color:#1F6B3A}
.cc-pill.bad{background:#F2E3E3;color:#A33}
.cc-pill.warn{background:#FBE3D5;color:#A34A00}
.cc-pill.off{background:#EDEDED;color:#666}

.cc-table .cc-no{color:var(--muted);font-size:12px;font-variant-numeric:tabular-nums;text-align:center}
.cc-sortable{cursor:pointer;user-select:none;white-space:nowrap}
.cc-sortable:hover{background:var(--surface-2,#F5F6F8)}
.cc-sortable.on{color:#C24C00}
.cc-arrow{margin-left:5px;font-size:9px;opacity:.55}
.cc-sortable.on .cc-arrow{opacity:1}
.cc-tick{display:inline-flex;align-items:center;gap:5px;font-size:12.5px}
.cc-tick-b{font-size:10.5px;padding:1px 6px;border-radius:6px;background:#EEF3FF;color:#33529E;font-weight:600}
.cc-key{border:1px solid var(--line,#E5E7EB);background:#fff;border-radius:7px;padding:3px 8px;
  font-size:11.5px;font-variant-numeric:tabular-nums;cursor:pointer;color:var(--muted)}
.cc-key:hover{border-color:#FA6400;color:#C24C00}

.cc-sec{margin:18px 0 9px;font-size:12.5px;font-weight:700;letter-spacing:.02em;
  padding-bottom:7px;border-bottom:1px solid var(--line,#E5E7EB)}
.cc-sec:first-child{margin-top:0}
.cc-check{display:inline-flex;align-items:center;gap:7px;font-size:13px;cursor:pointer;user-select:none}
.cc-check input{width:15px;height:15px;accent-color:#FA6400;cursor:pointer}
.cc-ticketgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(118px,1fr));gap:9px 12px;margin-top:10px}
@media (max-width:640px){
...
/* 센터 상세 — 전체화면 */
.cc-page{margin-top:8px;padding-bottom:56px}
.cc-back{border:0;background:transparent;padding:6px 0;font-size:13px;color:var(--muted);
  cursor:pointer;display:inline-flex;align-items:center;gap:4px}
.cc-back:hover{color:#C24C00}
.cc-hero{display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-top:6px}
.cc-hero-t{font-size:26px;font-weight:800;letter-spacing:-.03em;margin:0;line-height:1.25}
.cc-hero-sub{display:flex;gap:14px;flex-wrap:wrap;margin-top:6px;font-size:13px;color:var(--muted)}
.cc-hero-r{margin-left:auto;display:flex;flex-direction:column;align-items:flex-end;gap:6px}
.cc-hero-tags{display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-top:14px}
.cc-tabs{display:flex;gap:2px;border-bottom:1px solid var(--line,#E5E7EB);margin:20px 0 22px;
  overflow-x:auto;position:sticky;top:0;z-index:5;background:var(--bg,#fff)}
.cc-tab{border:0;border-bottom:2px solid transparent;background:transparent;padding:10px 16px;
  font-size:14px;cursor:pointer;color:var(--muted);white-space:nowrap;margin-bottom:-1px;
  display:inline-flex;align-items:center;gap:6px}
.cc-tab:hover{color:inherit}
.cc-tab.on{color:#C24C00;font-weight:700;border-bottom-color:#FA6400}
.cc-tab .cc-cnt{background:#FFF1E6;color:#C24C00;border-radius:999px;padding:0 7px;
  font-size:11.5px;font-weight:700;line-height:18px}
.cc-tab:not(.on) .cc-cnt{background:var(--surface-2,#F1F1EF);color:var(--muted)}

.cc-grid{display:grid;grid-template-columns:minmax(280px,340px) 1fr;gap:18px;align-items:start}
.cc-side{display:flex;flex-direction:column;gap:14px;position:sticky;top:64px}
.cc-panel{border:1px solid var(--line,#E5E7EB);border-radius:14px;padding:18px 20px;
  background:var(--surface,#fff)}
.cc-panel.cc-wide{padding:20px 22px}
.cc-panel h2{font-size:12.5px;font-weight:700;letter-spacing:.1em;color:var(--muted);
  margin:0 0 12px;text-transform:none}
.cc-panel-h{display:flex;align-items:baseline;gap:10px;margin-bottom:12px}
.cc-panel-h h2{font-size:12.5px;font-weight:700;letter-spacing:.1em;color:var(--muted);margin:0}
.cc-main{border:1px solid var(--line,#E5E7EB);border-radius:14px;padding:18px 20px;
  background:var(--surface,#fff);min-height:240px}
.cc-kv{display:flex;gap:10px;font-size:13px;line-height:1.6;padding:4px 0}
.cc-kv>span{color:var(--muted);flex:0 0 88px}
.cc-kv>b{font-weight:600;word-break:break-all;min-width:0}
.cc-kv i{font-style:normal;color:var(--muted);font-weight:400}
.cc-pbig{font-size:32px;font-weight:700;letter-spacing:-.03em;font-variant-numeric:tabular-nums;
  line-height:1.1}
.cc-pbig em{font-style:normal;font-size:14px;font-weight:600;color:var(--muted);margin-left:3px}
.cc-prow{display:flex;gap:22px;margin-top:12px}
.cc-prow span{display:block;font-size:11.5px;color:var(--muted)}
.cc-prow b{font-size:15px;font-weight:700;font-variant-numeric:tabular-nums}
.cc-prow b.hot{color:#C0392B}
...
.rnd-drop{display:block;text-align:center;padding:16px;border:1.5px dashed var(--line);
  border-radius:10px;cursor:pointer;font-size:13px;color:var(--muted);background:var(--surface-2,#F5F6F8);
  transition:.15s}
...
.rnd-table tbody tr:hover{background:var(--surface-2,#F5F6F8)}
.rnd-table tbody tr.on{background:var(--accent-soft,#FBEAE1)}
...
.rnd-modal-in{width:100%;max-width:720px;background:var(--surface,#fff);border:1px solid var(--line);
...
.rnd-modal-hd{display:flex;align-items:center;gap:8px;flex-wrap:wrap;
  padding:12px 16px;border-bottom:1px solid var(--line);background:var(--surface-2,#F5F6F8)}
