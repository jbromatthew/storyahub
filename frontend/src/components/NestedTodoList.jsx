    ...
    <div className="nt-list">
      {!compact && todos.length > 0 && (
        <div className="nt-hint">
          대분류 {totalDone}/{todos.length} 완료 · 소분류를 다 끝내면 대분류가 자동 완료돼요
        </div>
      )}
      ...
      {todos.map((t) => {
        ...
        return (
          <div key={t.id} className={"nt-card" + (done ? " done" : "")}>
            <div
              className="nt-phead"
              onClick={() => {
            ...
            >
              {hasSubs ? (
                <span
                  className={"nt-chev" + (isOpen ? " open" : "")}
                  onClick={(e) => toggleOpen(t.id, e)}
                >
                  <Chevron />
                </span>
              ) : (
              ...
              {!hasSubs && t.due && t.due !== "-" && !isOpen && (
                <span className="small" style={{ flex: "0 0 auto", fontSize: 11 }}>
                  {t.due}
                </span>
              )}
              {!compact && (
