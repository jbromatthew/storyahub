  ...
  app.use(
    cors({
      origin(origin, cb) {
        ...
        // 2) allowlist 설정이 누락/변경돼도 자사 도메인 서브도메인은 허용
        //    storyahub.com(record/app/api) + broj.io(b2b 등). Origin 헤더는 항상 scheme+host만 포함.
        if (/^https:\/\/([a-z0-9-]+\.)*storyahub\.com$/i.test(origin)) return cb(null, true);
        if (/^https:\/\/([a-z0-9-]+\.)*broj\.io$/i.test(origin)) return cb(null, true);
